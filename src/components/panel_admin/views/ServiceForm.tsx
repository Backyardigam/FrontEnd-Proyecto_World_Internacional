...
interface FormError {
  source: "general" | "background" | "galery" | "routes" | "card" | "schedule";
  message: string;
}
...
export default function ServiceForm({
  ...
  const [existingImages, setExistingImages] = useState<
    ServiceGet["service"]["mediaFiles"] | null
  >(null);
  ...
  /**
   * Convierte una cadena de tiempo en formato 12h (ej: "8:00 am", "5:00 pm")
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (limit && totalCurrentImageCount + newFilesArray.length > limit) {
      setError({
        source: name as any,
        message: `Límite de ${limit} archivo(s) excedido. Tienes ${totalCurrentImageCount} y trataste de añadir ${newFilesArray.length}.`,
      });
  ...
  const handleToggleDeleteUrl = (url: string) => {
    ...
    if (isRestoring) {
      ...
      if (existingCardImages.includes(url)) targetInputName = "card";
        ...
        [
          ...getExistingUrlsFor("urlBg1"),
          ...getExistingUrlsFor("urlBg2"),
        ].includes(url)
      )
        targetInputName = "background";
      else if (getExistingUrlsFor("urlTrip").includes(url))
        targetInputName = "routes";
      ...
      if (targetInputName) {
        ...
        if (limit && currentCount >= limit) {
          setError({
            source: targetInputName as any,
            message: `No se puede restaurar. El límite de ${limit} archivo(s) para esta sección ya está alcanzado.`,
          });
  ...
  const getExistingUrlsFor = (
    section: keyof NonNullable<ServiceGet["service"]["mediaFiles"]>
  ): string[] => {
    ...
    return Array.isArray(urlsOrString)
      ? urlsOrString.filter(Boolean)
      : urlsOrString
      ? [urlsOrString]
      : [];
  };
  ...
  const getActiveImageCount = (
    inputName: "background" | "routes" | "card" | "galery"
  ): number => {
    ...
    const activeExistingCount = existingUrls.filter(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // --- VALIDACIÓN FINAL DE HORARIOS ---
    if (schedule.length === 0) {
      setError({
        source: "schedule",
        message: "Debe haber al menos un horario.",
      });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setError({
        source: "general",
        message: err.message || "Ocurrió un error al guardar el servicio.",
      });
    } finally {
      setLoading(false);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="p-6 border border-gray-200 rounded-lg space-y-6">
          <h3 className="text-xl font-semibold text-gray-800 border-b pb-3">
            Detalles de la Página del Servicio
          </h3>
            ...
            {!isEditMode && (
              <div>
                ...
                <select
                  id="serviceState"
                  name="serviceState"
                  value={formData.serviceState || "visible"}
                  onChange={handleChange}
                  className="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md"
                >
          ...
          <div>
            <label
              ...
              className="block text-sm font-medium text-gray-700"
            >
              Información Adicional (una por línea)
            </label>
            <textarea
              id="additional"
              name="additional"
          ...
          <ScheduleManager
            ...
            error={error?.source === "schedule" ? error.message : null}
          />
          <div>
            ...
            <div className="space-y-6 mt-2">
              <ImageManager
                ...
                errorMessage={
                  error?.source === "background" ? error.message : null
                }
                existingUrls={[
                  ...getExistingUrlsFor("urlBg1"),
                  ...getExistingUrlsFor("urlBg2"),
                ...
                onRemoveNewUrl={(file) =>
                  handleRemoveNewFile("background", file)
                }
        ...
        <div className="p-6 border border-gray-200 rounded-lg space-y-6">
          <h3 className="text-xl font-semibold text-gray-800 border-b pb-3">
            Detalles de la Tarjeta del Servicio
          </h3>

          <div>
            <label
              htmlFor="compactDescription"
              className="block text-sm font-medium text-gray-700"
            >
            ...
            <textarea
              ...
              value={formData.compactDescription || ""}
              onChange={handleChange}
              rows={2}
              className="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md"
            />
